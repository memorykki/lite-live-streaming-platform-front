      ...
      <el-tabs :tab-position="tabPosition" style="height: 100%;">
        <el-tab-pane label="我的动态">
          <el-row :gutter="10">
            <!-- 添加 -->
            <el-col :span="4">
              <el-button type="primary" plain icon="el-icon-plus" size="small" @click="dialogVisible = true">发布
              </el-button>
              <el-dialog title="发布动态" :visible.sync="dialogVisible" width="30%">
                <el-form ref="dynamic" :model="dynamic" label-width="80px">
                  <el-form-item label="用户">
                    <el-input :disabled="true" v-model="dynamic.userId"></el-input>
                  </el-form-item>
                  ...
                  <el-form-item v-if="dynamic.dynamicType==1" label="内容">
                    <div>
                      <el-input type="textArea" v-model="dynamic.dynamicContent"></el-input>
                    </div>
                  </el-form-item>
                  <el-form-item v-else-if="dynamic.dynamicType==2" label="内容">
                    <div>
                      请选择图片：
                      <el-upload class="upload-demo" action="上传的后端接口" :on-preview="handlePreview"
                        :on-remove="handleRemove" :file-list="fileList" list-type="picture" limit="1">
                        <el-button size="small" type="primary">点击上传</el-button>
                        <div slot="tip" class="el-upload__tip">只能选择一张图片，只能上传jpg/png文件，且不超过500kb</div>
                      </el-upload>
                    </div>
                  </el-form-item>
                  <el-form-item v-else-if="dynamic.dynamicType==3" label="内容">
                    <div>
                      请选择视频：
                      <el-upload class="avatar-uploader" action="上传地址" v-bind:data="{FoldPath:'上传目录',SecretKey:'安全验证'}"
                        v-bind:on-progress="uploadVideoProcess" v-bind:on-success="handleVideoSuccess"
                        v-bind:before-upload="beforeUploadVideo" v-bind:show-file-list="false" limit="1">
          ...
          <!-- 动态 -->
          <el-row :span="8" v-for="(item, index) in tableData" :key="index" style="margin-bottom: 30px">
            <div>
              <p style="margin: 0;font-weight: bolder;font-size: larger;" id="title">{{ item.userId }}</p>
              <p style="margin: 0;">时间:{{ item.createTime }}</p>
            </div>
            <div>
              <div v-if="item.dynamicType===1">{{item.dynamicContent}}</div>
              <div v-else-if="item.dynamicType===2">
                <el-image :src="item.dynamicContent" fit="fill" style="width: 100%; height: 100%; border-radius: 8px">
                </el-image>
              </div>
              <div v-else-if="item.dynamicType===3">
                <p>
                  这是一条视频动态：<el-link href="item.dynamicContent" target="_blank">视频链接</el-link>
                </p>
              </div>
            </div>
            <div>
              <el-row :gutter="10">
                ...
                <el-col span="4">
                  <el-tooltip content="点击删除" placement="bottom">
                    <el-button size="small" @click="remove(scope.row)" type="danger" circle icon="el-icon-delete">
                    </el-button>
                  </el-tooltip>
                </el-col>
                <!-- <el-col span="1">
