<template>
  <div>
    <el-card>
      <div slot="header" shadow="always">
        ...
        <el-breadcrumb separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item>直播回放</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      <!-- layout布局存放工具栏 -->
      ...
      <div id="el_table">
        ...
        <el-table
          ...
          height="500"
        >
          <el-table-column prop="playbackId" label="回放ID"> </el-table-column>
          <el-table-column prop="roomId" label="房间号"> </el-table-column>
          <el-table-column prop="playbackPath" label="地址"> </el-table-column>
    ...
    <el-dialog title="回放管理" :visible.sync="dialogFormVisible">
      <el-form :model="user">
       
        <el-input v-model="user.roomId" placeholder="请输入房间号"></el-input>
        <el-input
          v-model="user.playbackPath"
          placeholder="请输入地址"
        ></el-input>
      </el-form>
    ...
    <!-- 添加对话框 -->
    <el-dialog title="添加回放信息" :visible.sync="dialogAddUser">
...
export default {
  ...
  methods: {
    ...
    add() {
      this.$http
        .post(
          "http://172.29.3.78:8081/lite-live-streaming-platform/playback/",
          this.addUser
        )
        .then((res) => {
          //关闭对话框
    ...
    findPage() {
      ...
      if (this.user_id) {
        //后端查询需要传递的参数
        params.roomId = this.user_id;
      }

      this.$http
        .get("http://172.29.3.78:8081/lite-live-streaming-platform/playback/", {
          params,
        })
        .then((res) => {
          //  将得到的数据赋值,可能出问题记得更改*************************************************
